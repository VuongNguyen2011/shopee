...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    $data = $warehouse->getNewPro();
    while ($result = $data->fetch_assoc()){
        $insertWare = $warehouse->insert_ware($_POST,$_FILES,$result['productID']);
    }
}
?>
...
<div class="grid_10">
    <div class="box round first grid">
        ...
        <div class="block">
            ...
            <form action="warehouseadd.php" method="POST" enctype="multipart/form-data">
                <table class="form">
                    ...
                    <tr>
                        ...
                        <td>
                            <select id="select" name="ncc">
                                ...
                                $ncc = $ncc->show_ncc();
                                if ($ncc) {
                                    while ($result = $ncc->fetch_assoc()) {
                                    $temp = $result['id']."/".$result['name'];
                                ?>
                                        <option value="<?php echo $temp ?>"><?php echo $result['name'] ?>
                                    </option>
                    ...
                    <tr>
                        ...
                        <td>
                            <input id="qty" name="qty" type="text" placeholder="Enter Qty..." class="medium" />
                        </td>
                    </tr>
...
<script>
    function cost1() {
        ...
        document.querySelector('#cost').value = (parseFloat(price.value) + (parseFloat(price.value)*parseFloat(interest.value))/100)*quantity.value; 
