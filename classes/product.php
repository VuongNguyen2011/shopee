...
class product
{
    ...
    public function insert_product($data,$file)
    {
        ...
        $supplier = mysqli_real_escape_string($this->db->link, $data['ncc']);
        $interest = mysqli_real_escape_string($this->db->link, $data['interest']);
        $temp = explode('/',$supplier);
        $supplier = $temp[1];
        ...
        if ($productName == "" || $brandID == ""|| $qty == "" || $catID == "" || $description == ""|| $supplier == "" || $file_name == "" || $price == ""|| $interest == "" ) {
            $alert = "<span class='error'>Field can not empty</span>";
            return $alert;
        } else {
            move_uploaded_file($file_temp,$uploaded_image);
            $query = "INSERT INTO tbl_product(`productName`, `catID`, `brandID`, `description`, `price`, `img` ,`cost`, `supplier`, `interest`,`qty`) VALUES('$productName','$catID','$brandID','$description','$price','$file_name','$cost','$supplier','$interest','$qty')";
    ...
    public function update_product1($data,$file,$id){
        $productName = mysqli_real_escape_string($this->db->link, $data['productName']);
        $catID = mysqli_real_escape_string($this->db->link, $data['category']);
        $brandID = mysqli_real_escape_string($this->db->link, $data['brand']);
        $description = mysqli_real_escape_string($this->db->link, $data['description']);
        ...
        if ($productName == "" || $brandID == "" || $catID == "" || $description == ""|| $supplier == "" || $price == ""|| $interest == "" ) {
            ...
            if(!empty($file_name)){
                move_uploaded_file($file_temp,$uploaded_image);
                $query = "UPDATE `tbl_product` SET `productName`='$productName',`catID`='$catID',`qty`='$qty',`brandID`='$brandID',`description`='$description',`price`='$price',`img`='$file_name',`interest`='$interest',`cost`='$cost',`supplier`='$supplier' WHERE productID = '$id'";

    
            }else{
                // neu nguoi dung khong chon anh
                $query = "UPDATE `tbl_product` SET `productName`='$productName',`catID`='$catID',`qty`='$qty',`brandID`='$brandID',`description`='$description',`price`='$price',`interest`='$interest',`cost`='$cost',`supplier`='$supplier' WHERE productID = '$id'";
            }     
            $result = $this->db->update($query);
            
            $query1 = "INSERT INTO tbl_warehouse(`productID`, `qty`, `priceOld`, `priceNew`, `productName`, `nccID`, `date`) 
            VALUES('$id','$qty','$price','$cost','$productName','2','$date')";
