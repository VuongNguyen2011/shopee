...
class warehouse
{
    ...
    public function insert_ware($data,$file,$id)
    {
        $productID = $id;
        $qty = mysqli_real_escape_string($this->db->link, $data['qty']);
        $priceOld = mysqli_real_escape_string($this->db->link, $data['price']);
        $priceNew = mysqli_real_escape_string($this->db->link, $data['cost']);
        $productName = mysqli_real_escape_string($this->db->link, $data['productName']);
        $nccID = mysqli_real_escape_string($this->db->link, $data['ncc']);
        $temp = explode('/',$nccID);
        $nccID = $temp[0];
        $date = date("Y-m-d h:i:sa");
        

        if ($qty == "" || $priceOld == ""|| $productName == "" || $priceNew == "") {
            $alert = "<span class='error'>Field can not empty</span>";
            return $alert;
        } else {
            // move_uploaded_file($file_temp,$uploaded_image);
            $query = "INSERT INTO tbl_warehouse(`productID`, `qty`, `priceOld`, `priceNew`, `productName`, `nccID`, `date`) 
            VALUES('$productID','$qty','$priceOld','$priceNew','$productName','$nccID','$date')";
            
            $result = $this->db->insert($query);

            if ($result) {
                $alert = "<span class='success'>Create Ware Successfully </span>";
                return $alert;
            } else {
                $alert = "<span class='error'>Create Ware Not Success</span>";
                return $alert;
            }
        }
    }
    ...
    public function show_warehouse(){
        $query = "CALL `getwarehouse`()";
        $result = $this->db->select($query);
        return $result;
    }
    public function show_warehouse1($from, $to){
        $query = "SELECT * FROM `tbl_warehouse` as w, tbl_ncc as c WHERE w.nccID = c.id AND cast(w.date as DATE) BETWEEN '$from' AND '$to';";
        $result = $this->db->select($query);
        return $result;
    }
